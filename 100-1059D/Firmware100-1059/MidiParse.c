...
char GateState=0;
char CurrentNote;
extern MESSBUFF *pDacTaskMsg;
extern MESSBUFF *pADSR1Msg;
extern MESSBUFF *pADSR2Msg;
...
void ActionSysExEnd(MIDIPARSE_D *pMD)
{
	...
	case SYSEX_CMD_GETCHANNEL_TRIG:
		break;
	case SYSEX_CMD_SEQ_LOAD:
		break;
	case SYSEX_CMD_SEQ_SAVE:
		break;
	}
}
...
{
	SetPortA(PORTA_MIDIRESET);
	PostSemaphore(pSemReset,0);
}

void ActionSysRTClock()
{
	SetPortC(PORTC_MIDI_CLOCK);
...
void ActionNoteOff(unsigned char chan,unsigned char Note, unsigned char Vel)
{
	...
	{
		PostMessage(pDacTaskMsg,DACTASK_UPDATEDAC,DAC_PITCH,(int)Note << 5);
		PostMessage(pADSR1Msg,ADSR_GATE_OFF,0,0);
		PostMessage(pADSR2Msg,ADSR_GATE_OFF,0,0);
	}
	else if (chan == Settings.m_CH2_Drum1)
	{

	}
	else if (chan == Settings.m_CH3_Drum2)
	{

	}
	else if (chan == Settings.m_CH4_Drum3)
	{

	}
	else if (chan == Settings.m_CH5_Cymbal)
	{
...
void ActionNoteOn(unsigned char chan,unsigned char Note, unsigned char Vel)
{
	...
	if(chan == Settings.m_CH1_MidiChannel)
	{
		//send out pitch
		PostMessage(pDacTaskMsg,DACTASK_UPDATEDAC,DAC_PITCH,(int)Note << 5);
		cmd = Vel?ADSR_GATE_ON:ADSR_GATE_OFF;
		PostMessage(pADSR1Msg,cmd,0,0);
		PostMessage(pADSR2Msg,cmd,0,0);
	}
	else if (chan == Settings.m_CH2_Drum1)
	{
		PostMessage(pDacTaskMsg,DACTASK_UPDATEDAC,DAC_DRUM1PITCH,(int)Note << 5);
	}
	else if (chan == Settings.m_CH3_Drum2)
	{
		PostMessage(pDacTaskMsg,DACTASK_UPDATEDAC,DAC_DRUM2PITCH,(int)Note << 5);
	}
	else if (chan == Settings.m_CH4_Drum3)
	{
		PostMessage(pDacTaskMsg,DACTASK_UPDATEDAC,DAC_DRUM3PITCH,(int)Note << 5);
	}
	else if (chan == Settings.m_CH5_Cymbal)
	{
		PostMessage(pDacTaskMsg,DACTASK_UPDATEDAC,DAC_CYMPITCH,(int)Note << 5);
	}
}
