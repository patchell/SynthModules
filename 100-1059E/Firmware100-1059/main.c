	...
	while(1)
	{
		...
		switch(pMSG->m_Cmd)
		{
			...
			case ADSR_GATE_ON:
				...
				WriteDAC(0x3f,DAC_ADSR1SUS);
				break;
			case ADSR_GATE_OFF:
				State = ADSR_STATE_RELEASE;
				WriteDAC(0,DAC_ADSR1SUS);
				break;
		}
		WriteADSRstate(State,0);
	}
...
void ADCTask(void *arg)
{
	...
	while(1)
	{
		...
		if(chan)
		{
			PostMessage(pDacTaskMsg,DACTASK_UPDATEDAC,DAC_ADSR1SUS,v);
			chan = 0;
		}
		else
		{
			PostMessage(pDacTaskMsg,DACTASK_UPDATEDAC,DAC_ADSR2SUS,v);
			chan = 1;
		}
		TimeDelay(50);
		ADMUX = chan;			//set ADC channel
